...
**1. Never Store User PINs**  
   - Unless explicitly building custodial wallets
   - Even then, requires enterprise-grade encryption

**2. Production Requirements**  
   - PINs must **never** be hardcoded
   - Client-side input only (never send to your servers)
   - Use secure enclaves/SHSM if storing

**3. Custodial vs Non-Custodial**  
   - 🛡️ **Custodial (Not Recommended):**  
     - Requires SOC 2 compliance
     - Regular third-party audits
     - User consent for key management
   - 🔑 **Non-Custodial (Preferred):**  
     - PIN never leaves user device
     - Client-side encryption only
     - Zero knowledge of user keys
